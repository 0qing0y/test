...
Page({
  ...
  getLandList: function () {
    ...
    wx.request({
      url: 'http://127.0.0.1:8080/testapp/api/land-list', // 替换为实际的后端接口地址
      method: 'GET',
      success: (res) => {
        console.log(res.data); // 打印后端返回的土地数据，确认数据格式与字段名
        this.setData({
          lands: res.data // 更新页面的土地数据
        });
      },
      fail: (err) => {
  ...
  viewDetails: function (e) {
    ...
    wx.navigateTo({
      url: `/pages/landdetail/landdetail?id=${id}`
    });
  },
...
calculateArea(boundaryPoints) {
  ...
  onon: function() {
    ...
    {
    // 在这里可以进行数据验证和提交处理
    console.log('确认按钮点击，提交数据：', this.data.onName, this.data.onQuantity, this.data.long,this.data.lat,this.data.onType, this.data.desc);
    const { onName, onQuantity  ,long,lat,onType, desc,boundaryPointsSend } = this.data
    const app = getApp();
    const u_id=app.globalData.u_id;
    wx.request({
      url: `http://127.0.0.1:8080/testapp/api/addland/?u_id=${u_id}`, // 替换为实际的后端接口地址
      method: 'PUT',

      data: {
        //f_id: onid,
        f_name: onName,         // 输入的土地名字
        f_square: onQuantity,     // 输入的面积（作为字符串处理）
        //f_add: onAdd,          // 输入的位置
        f_type: onType,
        f_longitude: long,
        f_latitude: lat,
      ...
      success: (res) => {
        ...
        this.refreshLandList(); // 添加成功后刷新土地列表
        this.seStockInModal();}
  ...
  refreshLandList: function () {
    this.getLandList(); // 重新获取土地数据
  },



  //导出信息
  exportLandData: function() {
    const app = getApp();
...
// 判断是否为管理员
    ...
    wx.request({
      url: 'http://127.0.0.1:8080/testapp/api/exportLand', // 替换为你的后端URL
      method: 'GET',
      ...
      success: function(res) {
        ...
        if (res.statusCode === 200) {
          const filePath = `${wx.env.USER_DATA_PATH}/Lands.xlsx`;
...
// 判断是否为管理员
  ...
  wx.chooseMessageFile({
    ...
    success: function(res) {
      ...
      wx.uploadFile({
        url: 'http://127.0.0.1:8080/testapp/api/importLand/',  // 替换为你的Django后端URL
...
fetchDataAndRenderChart: function() {
  ...
  wx.request({
    url: 'http://127.0.0.1:8080/testapp/api/getPlantingData', 
    method: 'GET',
    data: {
      startDate: startDate,
      endDate: endDate,
