const APIKEY = "b2b6a388d4e149c99e2cd8618d7eeea7";// 填入你申请的KEY
Page({
  ...
  getWeather() {
    ...
    wx.request({
      ...
      success(result) {
        ...
        wx.hideLoading();
        // 检查灾害提醒
        that.checkDisasterWarning(res.daily);
  ...
  formatNumber(n) {
   ...
   checkDisasterWarning(daily) {
    ...
    daily.forEach(day => {
      if (day.tempMax > 35) {
        warning = true;
        message = '高温警报：请注意防暑降温。';
      } else if (day.tempMin < 0) {
        warning = true;
        message = '低温警报：请注意保暖防寒。';
      } else if (day.windSpeed > 20) {
        warning = true;
        message = '大风警报：请注意防风加固设施。';
      } else if (day.precip > 20) {
        warning = true;
        message = '暴雨警报：请注意防范洪涝灾害。';
      }
    });
    this.setData({
      disasterWarning: warning,
